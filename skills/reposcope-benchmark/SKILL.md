...
3. Use only `repo_search`, `repo_read`, and `repo_context` to acquire repository source.
4. Keep one `sessionId` throughout the run.
5. Use normal editing tools after relevant source has been delivered through RepoScope. Editing itself is not the experimental variable.
6. Run only the benchmark-approved verification path. Do not inspect hidden verifier implementation unless the benchmark protocol explicitly permits it.
7. Finish with `repo_session_finish` and preserve its metrics for the benchmark record.
8. Record provider-observed token metrics only when actually available; never estimate missing provider billing/context numbers.
